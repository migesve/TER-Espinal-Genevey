        ...
        try {
            ...
            if (potentialLogin.rowCount > 0) {
                ...
                if (isSamePass) {
                    ...
                    return res.json({ LoggedIn: true, username: potentialLogin.rows[0].username, statut: potentialLogin.rows[0].statut, cohorte: potentialLogin.rows[0].cohorte });
                } else {
                    return res.json({ LoggedIn: false, status: "Email ou mot de passe incorrect !" });
                }
            } else {
                return res.json({ LoggedIn: false, status: "Email ou mot de passe incorrect !" });
            }
    ...
    try {
        ...
        if (existingUser.rowCount === 0) {
            ...
            if (existingEmail.rowCount === 0) {
                ...
                return res.json({ LoggedIn: true, username: req.body.username, statut: newUserQuery.rows[0].statut, cohorte: newUserQuery.rows[0].cohorte });
            } else {
                return res.json({ LoggedIn: false, status: "Email déjà utilisé" });
            }
        } else {
            return res.json({ LoggedIn: false, status: "Nom d'utilisateur déjà utilisé" });
        }
    } catch (err) {
        console.error(err);
    }
