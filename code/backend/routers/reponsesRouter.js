...
router.route("/getAll").get(async (req, res) => {
  const listeReponses = await pool.query("SELECT * FROM reponses", []);
  ...
  if (listeReponses.rowCount > 0) {
    ...
    return res.json({
      Succes: false,
      status: "La liste des inclinaisons n'a pas pu être récupérée !",
    });
  }
});

router.route("/getByPosition/:id").get(async (req, res) => {
  ...
  if (listeReponses.rowCount > 0) {
    ...
    return res.json({
      Succes: false,
      status: "La liste des reponses n'a pas pu être récupérée !",
    });
  }
});
...
router.route("/getCohortes").get(async (req, res) => {
...
router.route("/getByCohorte/:cohorte/:difficulte").get(async (req, res) => {
  ...
  if (!cohorte || !difficulte) {
    return res
      ...
      .json({
        success: false,
        status: "Les paramètres cohorte et difficulte sont requis.",
      });
  }
  ...
  try {
    ...
    const newReponse = await pool.query(
      ...
      [
        ...
        req.body.schema1_angle,
        req.body.schema1_inclinaison,
        ...
        req.body.corr_schema1_inclinaison,
        ...
        req.body.remarque_schema1_inclinaison,
