    ...
    try {
        ...
        if (existingSet.rowCount == 0) {
            const newSetQuery = await pool.query(
                'Insert INTO sets (nom, abreviation, descriptif, angle1, angle2) VALUES ($1, $2, $3, $4, $5) RETURNING position_id, nom, abreviation, descriptif, angle1, angle2, schema1_tete, schema1_bassin, schema1_fontanelle',
                [req.body.nom, req.body.abreviation, req.body.descriptif, req.body.angle1, req.body.angle2]
            );//modifier les paths pour les colonnes schéma1

            return res.json({ Succes: true, position_id: newSetQuery.rows[0].position_id, nom: newSetQuery.rows[0].nom, abreviation: newSetQuery.rows[0].abreviation, descriptif: newSetQuery.rows[0].descriptif, angle1: newSetQuery.rows[0].angle1, angle2: newSetQuery.rows[0].angle2});
        }
        else {
