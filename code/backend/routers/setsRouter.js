...
router.post('/upload', async (req, res) => {
    ...
    if(existingSet.rowCount == 0){
        ...
        res.json({ Succes : true, position_id : newSetQuery.rows[0].position_id, nom : newSetQuery.rows[0].nom, abreviation : newSetQuery.rows[0].abreviation, descriptif : newSetQuery.rows[0].descriptif, angle1 : newSetQuery.rows[0].angle1, angle2 : newSetQuery.rows[0].angle2, schema1_tete : newSetQuery.rows[0].schema1_tete, schema1_bassin : newSetQuery.rows[0].schema1_bassin, schema1_fontanelle : newSetQuery.rows[0].schema1_fontanelle});
    }
    else {
        res.json({ Succes : false, status : "Le set existe déjà !"});
    }
});
